...
export function unsyncedCount(input: SyncCloseInput): number {
  return (input.pendingCount ?? 0) + (input.errorCount ?? 0) + (input.deadCount ?? 0)
    + (input.invoiceCollectionPendingCount ?? 0) + (input.invoiceCollectionReviewCount ?? 0);
}
...
export function hasUnsyncedWork(input: SyncCloseInput): boolean {
  return input.invoiceCollectionSummaryReady === false || unsyncedCount(input) > 0 || input.isSyncing === true;
}
...
export function describeCloseSyncBlock(input: SyncCloseInput): string | null {
  if (input.invoiceCollectionSummaryReady === false) {
    return 'No se pudo verificar la cobranza pendiente antes de cerrar ruta.';
  }
  if (input.isSyncing) {
  ...
  const collectionPending = input.invoiceCollectionPendingCount ?? 0;
  const collectionReview = input.invoiceCollectionReviewCount ?? 0;
  if (pending + failed + collectionPending + collectionReview === 0) return null;
  const parts: string[] = [];
  if (pending > 0) parts.push(`${pending} pendiente(s)`);
  if (failed > 0) parts.push(`${failed} con error`);
  if (collectionPending > 0) parts.push(`${collectionPending} de cobranza pendiente(s) de confirmación`);
  if (collectionReview > 0) parts.push(`${collectionReview} de cobranza en revisión`);
